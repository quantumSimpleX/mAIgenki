...
export default function BodyMapScreen() {
  ...
  const handleRelocationPlace = useCallback(async (cx: number, cy: number) => {
    ...
    const cx_percent = (cx / 260) * 100
    const cy_percent = (cy / 460) * 100
    // console.log(`[Condition Relocated] ${relocatingCondition.label}: ${cx_percent.toFixed(2)}% x ${cy_percent.toFixed(2)}%`)
    if (db) await updateConditionPosition(db, relocatingCondition.id, cx_percent, cy_percent)
    refreshConditions()
    cancelRelocation()
