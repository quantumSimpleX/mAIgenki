...
- **Default state**: entering edit mode selects **Add** by default (highlighted) — a user can immediately tap the body to place a location without an extra step.
...
- **Remove mode**: selecting Remove highlights that control; while active, tapping/clicking an existing dot belonging to the condition being edited removes that location. Dots belonging to other conditions are not interactive while editing this condition. A condition must always retain at least one location — an attempt to remove the last remaining location is rejected with a brief inline message rather than leaving the condition with zero locations (and never crashes or silently no-ops without feedback).
...
- **Done**: tapping the checkmark exits location-editing mode and returns to the normal bodymap view with the just-edited condition selected (its sheet reopened) so the user can see the result of their edits. This mirrors today's cancel-relocation exit, except it lands on the sheet rather than a bare map.
