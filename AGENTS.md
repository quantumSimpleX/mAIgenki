...
**Storage is IndexedDB** (browser-native), via `src/lib/db/indexedDb.ts` and `indexedDbBackup.ts`. The project's original design used `expo-sqlite`, and that code (`src/lib/db/schema.ts`, `queries.ts`, `backup.ts`, all still present and targeting `expo-sqlite`) has not yet been fully migrated — see `doc.userDataFlow/userDataTask.md` for current migration status. Build new persistence work against IndexedDB; don't extend the `expo-sqlite` path. **No SQLite-to-IndexedDB data migration is needed or planned**: mAIgenki has not launched, so no user has ever saved a real record against the `expo-sqlite` path — there is no existing data to carry forward. The unmigrated `expo-sqlite` files exist only as legacy code pending removal, not as a live data source. If this ever changes (e.g. a pre-cutover build shipped to real users), a migration path must be added before relying on this assumption again.
