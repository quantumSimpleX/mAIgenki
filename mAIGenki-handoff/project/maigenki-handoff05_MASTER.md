# mAI Genki — Master Design Handoff 05
**Date:** June 27, 2026
**File:** `mAI Genki.dc.html` (~1600 lines)
**Design system:** QSXC designSys bound at `_ds/qsxc-designsys-609610ba-6924-415c-8ab5-533e3cf06c97/`
**Supersedes:** maigenki-handoff01 through 04. This document is authoritative. All conflicts resolved in favor of this file.

> DS warning in console is **expected** — mAI Genki is a standalone mobile app prototype; it does not consume the DS bundle components directly. Brand tokens, typography, and color palette are applied inline per the DC authoring model.
...
```
mAI Genki.dc.html           — single Design Component, all 3 screens (~1600 lines)
maigenki-handoff01–05.md    — session notes (this file is master)
fonts/
  MOMCAKE-Bold.otf
  MOMCAKE-Thin.otf
...
### QS wordmark placement
- **Upload screen:** lower-right, `opacity:0.4`, "Built by" label (Barlow Condensed 9px, uppercase, `#A6ADB7`) above it
- **Body map:** lower-left, `opacity:0.3`, no label, white version

The wordmark on the body map doubles as the toggle for the upload shortcuts panel — tap it to show/hide.
...
| Condition name (sheet) | MOMCAKE | 700 | 26px |
| UI labels / body | Barlow Condensed | 400–700 | 10–17px |
| Eyebrow / tab labels | Barlow Condensed | 600 | 10–12px, UPPERCASE, 0.1em tracking |
| Mono / dates / codes / source block | Source Code Pro | 400 | 9–13px |
...
| Token | Hex | Use |
|---|---|---|
| Amethyst | `#8A60EB` | Logo "AI", primary accent, chat button, legend active underline |
| Aqua | `#1FC3A4` | Time rail thumb, progress bar, renal system, contact icons |
...
| 4 | `cardio` | **Cardiovascular** | `#EF4444` |
...
| 11 | `repro` | Reproductive | `#C0526A` |

> **Note (session 05):** System #4 label is "Cardiovascular" — not "Circulatory" as in earlier handoffs.
...
> **Session 05 change:** The "Body Map | Timeline" tab bar has been **permanently removed**. The timeline list view and `bodyMapMode` state no longer exist. The body canvas goes directly from the top bar.
...
```
`right: 14px` = inactive rail width (centers figure correctly in available space).
...
Four 28×28px icon buttons (6px radius):

> **Session 05 change:** "Add records" (+ button) has been **permanently removed** from this panel.
...
### Height / animation (session 05 final)
- **Closed:** `transform: translateY(110%)` — slides completely off the bottom of the screen. The QS wordmark and body map are fully visible when closed.
- **Condition detail open:** `height: 400px`, `borderRadius: 18px 18px 0 0`, `transform: translateY(0)`
- **Chat open (full-screen):** `height: 780px`, `borderRadius: 0`, `transform: translateY(0)`
- Transition: `transform 420ms, height 420ms, border-radius 420ms cubic-bezier(0.16,1,0.3,1)`

> **Session 05 change:** Previously used `height: 0` when closed, which left an invisible element covering the body map. Now uses `translateY(110%)` to fully clear the screen.

- **↓ close button**: calls `closeSheet()` → dismisses back to body map
...
- **"First noted" date chip** (aqua tint, calendar icon, SCP 12px) — editable via pencil icon button. Tap → inline text input, confirm ✓/Enter, cancel ×. Stored in `condDateOverrides[condId]`.
- Clinical note (Barlow 400, 16px)
- **Source block** (see below)
- **Chat button** — lower-right of card, own footer row (`display:flex; justifyContent:flex-end; marginTop:16px`). Amethyst 36×36px, 8px radius. Tap → `chatOpen: true`.

### Source block (session 05 final — 3-line layout)

```
Line 1:  SOURCE  ·  City, State, Country
Line 2:  Full Institution Name
Line 3:  Dr. FirstName LastName  [✉]  [📞]
```

- **Line 1:** "SOURCE" label (Barlow Condensed 600, 10px, uppercase, 0.18em tracking, rgba(255,255,255,0.2)) + `·` separator + location (Source Code Pro 10px, rgba(255,255,255,0.32))
- **Line 2:** Institution name (Source Code Pro 10px, rgba(255,255,255,0.32))
- **Line 3:** Doctor name (Source Code Pro 13px, bold, rgba(255,255,255,0.65)) + email icon (aqua `#1FC3A4`, 15px, no box) + phone icon (aqua `#1FC3A4`, 15px, no box)

The file icon that was previously to the left of the source block has been **removed**.
The email and phone icon buttons have **no bounding box** — bare icons only, aqua stroke, prominent.
...
- **Tap selection circle** (upper-right, 18px, `e.stopPropagation()`) → multi-select toggle

**Selected card visual state:** filled circle + white ✓, 2px inset colored border + tint overlay, outer border glow

### Full-screen record lightbox
- Rendered at **phone-screen level** (`position:absolute; inset:0; z-index:100`)
- Covers entire phone including top bar
- Shows: type badge + label + enlarged thumbnail (358×226px) + date
- "Add to chat" / "✓ In chat" toggle button
- Tap anywhere / × button to dismiss
...
Pinned at top: 🇺🇸 EN · 🇹🇼 TW · 🇯🇵 JA · 🇪🇸 ES (thin separator below)
...
Each row: flag emoji + 2-letter code badge (SCP 9px) + native name (Barlow 14px) + English sub (SCP 9.5px muted)
Active row: amethyst 3px left border + amethyst tint bg + ✓ checkmark right

---

## Full state shape (session 05 final)
...
state = {
  ...
  // bodyMapMode removed in session 05 — Timeline list view eliminated
};
```
...
| vitd | endo | Vitamin D deficiency | 2015-JUL-08 | 162 | 113 |
| fibroid | repro | Uterine fibroids | 2020-JUN-22 | 130 | 402 |
| pcos | repro | PCOS | 2016-OCT-29 | 148 | 392 |

All conditions carry: `medName`, `localNames` (20 languages), `yearFrac` (computed), `note`, `evidence`.

---

## Evidence / source data (session 05 final — full names throughout)

All 22 conditions use full names — no abbreviations for physicians, institutions, or locations.

| Condition | Dr. Name | Institution | Location |
|---|---|---|---|
...
## Session 05 — Full change log

All changes below are final and supersede all earlier handoffs.

1. **Sheet dismissal** — `height: 0` → `transform: translateY(110%)`. Sheet now slides completely off the bottom of the screen when closed. QS wordmark and body map are fully unobstructed.
2. **Timeline list view removed** — The "Body Map | Timeline" tab bar, the list view overlay, and all associated state (`bodyMapMode`) and JS (`onBodyMapTab`, `onListTab`, `bodyMapListEl`, etc.) are permanently deleted. The app already has a time rail; a second timeline was redundant.
3. **System label rename** — `cardio` system renamed from "Circulatory" to "Cardiovascular".
4. **"Add records" button removed** — The + Add records button (aqua cross) from the upload shortcuts panel has been permanently removed.
5. **Source block redesigned** — 3-line layout (see above). File icon removed. Evidence string now stores `Dr. FullName · Institution · City, ST, Country` and is parsed at render time.
6. **Doctor names** — All 22 conditions use full first + last names (e.g. "Dr. Sarah Kim", not "Dr. S. Kim").
7. **Institution names** — All 22 conditions use full proper institution names (no abbreviations).
8. **Chat button relocated** — Moved from inline with the source block to its own footer row (`display:flex; justifyContent:flex-end; marginTop:16`) at the lower-right of the condition card.
9. **Contact icons** — Email and phone icon buttons are bare aqua (`#1FC3A4`) stroke icons, 15px, no bounding box. Displayed next to doctor's bold name on line 3 of source block.
10. **Doctor name styling** — Bold (`fontWeight:700`), 13px Source Code Pro, `rgba(255,255,255,0.65)`.
...
2. **Multi-PDF merge UI** — "upload shortcuts" currently re-runs analyze animation; real merge UX would diff and add new conditions without resetting existing ones
3. **Body centering with active rail** — SVG uses `right:14px` (inactive rail width). When rail expands to 36px the figure shifts slightly left; animate `right` in sync with rail width.
...
8. **Doctor contact actions** — email/phone icon buttons are visual only; wire to `mailto:` / `tel:` when real provider data is available
